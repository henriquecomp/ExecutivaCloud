...
export function normalizeExecutivePayload(executive: Partial<Executive>): Record<string, unknown> {
  ...
  return {
    ...
    zipCode: cleanText(rest.zipCode),
    number: cleanText(rest.number),
    neighborhood: cleanText(rest.neighborhood),
    city: cleanText(rest.city),
    state: cleanText(rest.state),
    complement: cleanText(rest.complement),
