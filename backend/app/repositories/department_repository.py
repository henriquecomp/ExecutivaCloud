...
class DepartmentRepository:
    ...
    def get_by_name_and_org(self, name: str, org_id: int) -> Optional[models.Department]:
        return self.db.query(self.model).filter(
            self.model.name == name,
            self.model.organizationId == org_id
        ).first()

    def get_by_organization_id(self, org_id: int) -> List[models.Department]:
        return self.db.query(self.model).filter(self.model.organizationId == org_id).all()
    ...
    def update(self, db_dept: models.Department, update_data: Dict[str, Any]) -> models.Department:
