...
from app.repositories.organization_repository import OrganizationRepository # Dependência
...
class DepartmentService:
    ...
    def create_department(self, dept_data: schemas.DepartmentCreate) -> models.Department:
        ...
        if self.repository.get_by_name_and_org(dept_data.name, dept_data.organizationId):
    ...
    def update_department(self, dept_id: int, update_data: schemas.DepartmentUpdate) -> models.Department:
        ...
        if "name" in update_dict or "organizationId" in update_dict:
            if (new_name != db_dept.name or new_org_id != db_dept.organizationId):
                ...
                if self.repository.get_by_name_and_org(new_name, new_org_id):
                    raise ValueError("Este nome de departamento já existe na empresa de destino.")
